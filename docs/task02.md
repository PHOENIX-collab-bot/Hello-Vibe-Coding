...
## 第二版
prompt：
- 请你在样式（至少三种主题：随着分数增加变换主题、难度随着分数增加难度越来越难：方块下降的速度越来越快、交互优化：优化布局、按钮、风格进行改进、
- 背景可以设置一下，浏览器访问时太空了，字体，按钮背景都要符合赛博朋克风格
- 现在还差一个记录历史最高分的功能，以及一个成就模块
- 现在功能时完善了但是新出现了一些bug，成就占的空间太大导致开始和暂停按钮消失了，要求成就以按钮形式及弹窗形式显示，且成就的滚轮条与游戏风格不符，显得十分突兀
- 还有一个问题：游戏界面存在裁切问题，最底下看不见，（网格盘的高度需要调整）
